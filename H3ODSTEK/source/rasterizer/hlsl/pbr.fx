...
float get_material_pbr_specular_power_scale(float power_or_roughness)
{
}	
	...
	out float3 analytic_specular_radiance)					// return specular radiance from this light				<--- ONLY REQUIRED OUTPUT FOR DYNAMIC LIGHTS
{
	...
	else
	{
		...
		else
		{
			...
			material_parameters.y = clamp((1 - material_parameters.x) * roughness_multiplier + roughness_bias, 0.005, 1.0);
