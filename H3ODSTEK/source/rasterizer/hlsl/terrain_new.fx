...
#elif MORPH_DYNAMIC(blend_type) == MORPH_pbr
...
{
#if SPECULAR_MATERIAL_COUNT > 0 || defined(_PBR_FX_)
...
{
    float3 ON = oren_nayar(albedo, view_dir, view_normal, view_light_dir, light_color, rough, fresnel);
	...
	inout float3 sh_spec_scalar)
{
	...
	float3 dif = on_and_sh_order2(albedo, ao, view_dir, view_normal, view_light_dir, light_color, rough, lighting_constants, fresnel, sh_spec_scalar);
	return dif;
}
...
//	#endif
	...
	float3 envmap_light= 0.0f;
		...
		diffuse_light = on_and_sh(
									...
									fresnel_analytical,
									specular_sh
									) * (1 - specular_pbr.metallic);
		simple_light_diffuse_light *=  (1 - specular_pbr.metallic);
	...
	// diffuse light	
#ifdef _PBR_FX_
	out_color.rgb= (diffuse_light + simple_light_diffuse_light) + (analytical_specular_light + simple_light_specular_light + envmap_light);
#else
...
//	#endif
	...
	float3 envmap_light= 0.0f;
		...
		float3 specular_sh = 0.0f;
		diffuse_light = on_and_sh_order2(
									albedo.xyz,
									specular_pbr.ao,
									view_dir,
									bump_normal,
	...
	bool cinematic)
{
	...
	if (dot(light_radiance, light_radiance) > 0.0000001f)									// ###ctchou $PERF unproven 'performance' hack
	{
		...
		analytic_diffuse_radiance= oren_nayar(
									diffuse_albedo,
									view_dir,
									bump_normal,
									fragment_to_light,
									light_radiance,
									roughness,
									fresnel_analytical) * (1 - specular_pbr.metallic);
	#else
...
//		slope= slope / dot(p_lighting_constant_1.xyz, fragment_to_light.xyz);				// adjust slope to be slope for z-depth
		...
		// diffuse light	
	#ifdef _PBR_FX_
		out_color.rgb= analytic_diffuse_radiance + analytic_specular_radiance;
	#else
