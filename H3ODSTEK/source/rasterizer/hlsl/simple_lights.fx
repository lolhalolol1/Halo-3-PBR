...
#ifdef _PBR_TOON_FX_
	light_radiance= LIGHT_COLOR * (round(combined_falloff * 4) / 4);								//										(1 instruction)
#else
	light_radiance= LIGHT_COLOR * combined_falloff;
#endif
}
		...
		out float3 specularly_reflected_light)
{
	...
	for (int light_index= 0; light_index < SIMPLE_LIGHT_COUNT; light_index++)
	{
		...
		*/
		...
		float3 ONdif =  albedo * (1 - fresnel) * (A + B * s / t) * (1 / PI) * light_radiance * NdotL;
		...
		out float3 specularly_reflected_light)
{
	...
	for (int light_index= 0; light_index < SIMPLE_LIGHT_COUNT; light_index++)
	{
        ...
        float3 H    = normalize(fragment_to_light + view_dir);
        float NdotL = dot(surface_normal, fragment_to_light);
        ...
        float NDFdenom = max((NdotH * a2_sqrd - NdotH) * NdotH + 1.0, 0.0001);
        float NDF = a2_sqrd / (pi * NDFdenom * NDFdenom);
		NDF = NDF >= 0.5 ? (round(max(NDF, 0.5f) * 2) / 2) : NDF;
		...
		*/
		...
		//Oren-Nayar diffuse
		diffusely_reflected_light	+= ONdif * (1 - metallic) * light_radiance;
	}
}
