...
#define SPECULAR_MATERIAL_PBR (SPECULAR_MATERIAL(material_0_type) == 2 || SPECULAR_MATERIAL(material_1_type) == 2 || SPECULAR_MATERIAL(material_2_type) == 2 || SPECULAR_MATERIAL(material_3_type) == 2)
...
#if MORPH_DYNAMIC(blend_type) == MORPH_dynamic
	PARAM(float4, dynamic_material);
	PARAM(float, transition_sharpness);
	PARAM(float, transition_threshold);
#endif // MORPH_DYNAMIC

#if MORPH_DYNAMIC(blend_type) == MORPH_pbr
	PARAM_SAMPLER_2D_ARRAY(orm_maps);
#endif
...
PARAM(bool, iridescent_m_##material_number);						\
PARAM(int, orm_index_m_##material_number);
DECLARE_MATERIAL(0);
DECLARE_MATERIAL(1);
DECLARE_MATERIAL(2);
DECLARE_MATERIAL(3);
...
float4 sample_blend_normalized(float2 texcoord)
{
	...
	/*#if SPECULAR_MATERIAL_PBR
		float final_h = max(max(blend.x, blend.y), max(blend.z, blend.w));
		blend.x = blend.x == final_h ? 1 : 0;
		blend.y = blend.y == final_h ? 1 : 0;
		blend.z = blend.z == final_h ? 1 : 0;
		blend.w = blend.w == final_h ? 1 : 0;
	#endif*/
...
{
#if SPECULAR_MATERIAL_COUNT > 0 || SELF_ILLUM_MATERIAL_COUNT > 0 || SPECULAR_MATERIAL_PBR
...
void calc_ggx_outgoing_light(
	...
	in float3	view_dir,
	// BRDF parameters
	in float	roughness, 			// artist-authored microfacet distribution value
	in float3	f0,					// specular reflectance at 0 degrees
	in float3   f90,				// specular reflectance at 90 degrees 
	in float	iridescence,		// blended material iridescence
	in float	fresnel_power,		// fresnel power for artist-driven iridescence
	// incident light parameters
	in float3	analytical_light_dir,
	in float3	analytical_light_color,
	// fresnel reflectance (used to conserve energy in the diffuse component)
	out float3 	fresnel,
	// outgoing light (towards view)
	out float3	analytical_specular_light)
{
    // calculate analyical specular light
	float NdotL = clamp(dot(normal_dir, analytical_light_dir), 0.0001, 1.0);
	float3 H = normalize(analytical_light_dir + view_dir);
	...
	float a2squared = a2 * a2;

	// Normalised Distribution Function 						(Based on the eqaution shown in "Moving Frostbite to Physically Based Rendering")
		//float distf = (NdotH * a2squared - NdotH) * NdotH + 1.0;
		//float NDF = a2squared / (distf * distf);

	float NDFdenom = max((NdotH * a2squared - NdotH) * NdotH + 1.0, 0.0001);
	float NDF = a2squared / (pi * NDFdenom * NDFdenom);

	// Visibility Function 										(Also based on what's shown in "Moving Frostbite to Physically Based Rendering")
		//float g_a = NdotV * sqrt(a2 + NdotL * (NdotL + a2 * -NdotL));
		//float g_b = NdotL * sqrt(a2 + NdotV * (NdotV + a2 * -NdotV));
		//float Vis = 0.5f / (g_a + g_b);

    float g_L = 2.0 * NdotL / (NdotL + sqrt(a2squared + (1.0 - a2squared) * (NdotL * NdotL)));
	float g_V = 2.0 * NdotV / (NdotV + sqrt(a2squared + (1.0 - a2squared) * (NdotV * NdotV)));
    float G = g_L * g_V;

	// Fresnel-Schlick
	float3 Fresnel_Schlick 	= f0 + (1.0 - f0) * pow(1.0 - VdotH, 5.0);
	// Fresnel-Schlick F90
	// 		-Oli:	F90 isn't actually an accurate way of handling edge colours for metals or iridescent surfaces. 
	//				Maybe look into F82? ("Fresnel Equations Considered Harmful" by Naty Hoffman, 2019)
		float3 Fresnel_F90 		= f0 + (f90 - f0) * pow(1.0 - VdotH, fresnel_power);

	//		-Oli: 	Calculating fresnel twice is not efficient, but for how Halo 3 handles blended materials,
	//				it's sort of necessary. Look into some way of doing this better (lerp F90 by iridescence?)
	float3 f = lerp(Fresnel_Schlick, Fresnel_F90, iridescence);
	fresnel = f;


	analytical_specular_light = (NdotV != 0.0f) ? ((NDF * f * G) / max(4.0 * NdotV * NdotL, 0.0001)) * analytical_light_color * NdotL : 0.00001f;
}
...
)
{
	...
	float3 ONdif =  albedo * (1 - fresnel) * (on_A + on_B * s / t) * (1 / pi) * light_color * NoL;

	return ONdif;
}

float3 lambert_and_sh(
	in float3 albedo,
	in float3 view_dir,
	in float3 view_normal,
	in float3 view_light_dir,
	...
	in float3 fresnel,
	out float3 sh_spec_scalar)
{
	float3 ON = oren_nayar(view_dir, view_normal, view_light_dir, light_color, fresnel, rough, albedo);
	//float3 ON = NdotL * (1 / pi) * (1 - fresnel);
	...
	sh_spec_scalar = lightprobe_color;
	return ON + lightprobe_color * albedo;
}

float3 lambert_and_sh_order2(
	in float3 albedo,
	in float  ao,
	in float3 view_dir,
	in float3 view_normal,
	...
	out float3 sh_spec_scalar)
{
	...
	//float sh_col_max = max(lightprobe_color.r, max(lightprobe_color.g, lightprobe_color.b));
	//float sh_scalar = (round(sh_col_max * 30) / 30) / sh_col_max;
	sh_spec_scalar = lightprobe_color + NdotL;
	return ON + lightprobe_color * albedo;
}
...
#define ACCUMULATE_MATERIAL_ALBEDO_AND_BUMP_PBR(material, blend_amount, albedo_accumulate, blendweight, bump)													\
COMPILER_IFANY																																					\
if (blend_amount > 0.0)																																			\
{																																								\
	float4 base=	sampleBiasGlobal2D(base_map_m_##material,	transform_texcoord(original_texcoord.xy, base_map_m_##material##_xform));						\
	...
	in float3 tangent : TEXCOORD3)
{
	...
	#if MORPH_DYNAMIC(blend_type) == MORPH_pbr
		//This is fucking abysmal, but we have to sample these in two different functions for height-blend due to how Bungie set this up.
...
struct specular_parameters_pbr
{
	float3 f0;
	float3 f90;
	float roughness;					// specular power
	float metallic;
	float ao;					// envmap contribution		(* specular contribution)
	float iridescent;
...
#define BLEND_SPECULAR_PBR(material_postfix, blend_amount, specular_pbr)				\
	blend_specular_parameters_pbr(														\
		blend_amount,																	\
		albedo,																			\
		specular_tint_m_##material_postfix,												\
		orm.x,																			\
		orm.y,																			\
		orm.z,																			\
		orm.w,																			\
		fresnel_curve_steepness_m_##material_postfix,									\
		specular_pbr)
...
void blend_specular_parameters_pbr(
	in float blend_amount,
	in float3 albedo,
	in float3 f90_tint,
	in float ao,
	...
	in float iridescent,
	in float fresnel_steepness,
	inout specular_parameters_pbr specular)
{
	specular.f0			= lerp(0.04, albedo, metallic);
	specular.f90		= f90_tint;
	
	specular.roughness				= roughness;
	specular.metallic				= metallic;
	specular.ao						= ao;
	specular.iridescent				= 0;				//Oli: Need to create a separate variable for this or remove it (implement chameleon instead?)
		
	specular.fresnel_steepness		= fresnel_steepness;
	specular.weight					= blend_amount;
}
...
void blend_surface_parameters(
	...
	out specular_parameters_pbr specular_pbr,
	out float3 self_illum)
{
	...
	#if MORPH_DYNAMIC(blend_type) == MORPH_pbr
	float4 orm = 0;
	specular_pbr.f0			= 0.0f;
	specular_pbr.f90		= 0.0f;
	specular_pbr.roughness	= 0.0f;
	specular_pbr.metallic	= 0.0f;
	specular_pbr.ao			= 0.0f;
	specular_pbr.iridescent = 0.0f;
	specular_pbr.fresnel_steepness		= 0.0f;
	specular_pbr.weight					= 0.0f;

	specular.normal_albedo= 0.0f;
	specular.power= 0.0f;				// default power is 1.0, default weight is 0.001
...
{	
	...
	specular_parameters_pbr specular_pbr;
	float3 self_illum;
	...
	float3 simple_light_specular_light;//= 0.0f;
		...
		float3 fragment_position_world= Camera_Position_PS - fragment_to_camera_world;
			...
			calc_simple_lights_ggx(
					...
					specular_pbr.f0,
					specular_pbr.f90,
					specular_pbr.fresnel_steepness,
					specular_pbr.roughness,
					specular_pbr.metallic,
					albedo,
	...
	float3 envmap_light= 0.0f;
		...
		float n_dot_v			= saturate(dot(bump_normal, view_dir));

		float roughness 	= specular_pbr.roughness;
		float3 f0			= specular_pbr.f0;
		float3 f90			= specular_pbr.f90;
		float3 iridescent	= specular_pbr.iridescent;

		float3 fresnel_analytical = 0.0f;
		...
		float3 specular_sh = 0.0f;
		diffuse_light = lambert_and_sh(
										albedo.xyz,
										view_dir,
										bump_normal,
										dominant_light_direction,
		...
		//area_specular_light= ravi_order_3(view_reflect_dir, sh_lighting_coefficients);//ravi_order_3(view_reflect_dir, sh_lighting_coefficients);
		//area_specular_light= max(0.0f, area_specular_light);

		float3 Fresnel_Gloss_env	= f0 + (max(1 - roughness, f0) - f0) * pow(1.0f - n_dot_v, 5.0f);
		float3 Fresnel_F90_env		= f0 + (f90 - f0) * pow(1.0f - n_dot_v, specular_pbr.fresnel_steepness);
		specular_tint				= lerp(Fresnel_Gloss_env, Fresnel_F90_env, iridescent);

		envmap_light= CALC_ENVMAP(envmap_type)(
			view_dir,
			bump_normal,
			view_reflect_dir,
			float4(EnvBRDFApprox(Fresnel_Gloss_env, roughness, n_dot_v) * specular_sh, roughness),		// convert specular power to roughness (cheap and bad approximation) EnvBRDFApprox(specular_tint, specular_pbr.roughness, n_dot_v) * 
			1);
	#elif SPECULAR_MATERIAL_COUNT > 0
	...
	// specular light
	#if MORPH_DYNAMIC(blend_type) == MORPH_pbr
		out_color.rgb += 	(envmap_light +
							(simple_light_specular_light + analytical_specular_light));
	#elif SPECULAR_MATERIAL_COUNT > 0
...
{	
	...
	float3 envmap_light= 0.0f;
		...
		float3 specular_sh = 0.0f;
		diffuse_light = lambert_and_sh_order2(
										albedo.xyz,
										specular_pbr.ao,
										view_dir,
										bump_normal,
	...
	// diffuse light	
	#if MORPH_DYNAMIC(blend_type) == MORPH_pbr
	out_color.rgb= (diffuse_light + simple_light_diffuse_light) * diffuse_coefficient;
	#else
	out_color.rgb= albedo.rgb * (diffuse_light + simple_light_diffuse_light) * diffuse_coefficient;
	#endif
	// specular light
	...
	bool cinematic)
{
	...
	if (dot(light_radiance, light_radiance) > 0.0000001f)									// ###ctchou $PERF unproven 'performance' hack
	{
		...
		// get diffuse albedo and specular mask (specular mask _can_ be stored in the albedo alpha channel, or in a seperate texture)
			...
			analytic_diffuse_radiance= oren_nayar(view_dir, bump_normal, fragment_to_light, light_radiance, fresnel_analytical, roughness, diffuse_albedo.rgb) * (1 - specular_pbr.metallic);
			// calculate full specular tint
			//float3 f0= specular_pbr.f0;
			float3 specular_sh = 0.0f;
		#else
...
//		slope= slope / dot(p_lighting_constant_1.xyz, fragment_to_light.xyz);				// adjust slope to be slope for z-depth
	...
	// specular light
	#if MORPH_DYNAMIC(blend_type) == MORPH_pbr
		out_color.rgb += analytic_specular_radiance;
	#elif SPECULAR_MATERIAL_COUNT > 0
		// calculate full specular tint
