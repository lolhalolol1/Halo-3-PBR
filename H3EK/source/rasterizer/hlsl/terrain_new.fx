...
{	
	...
	float3 envmap_light= 0.0f;
		...
		area_specular_light= diffuse_light;//ravi_order_3(view_reflect_dir, sh_lighting_coefficients);
		...
		envmap_light= CALC_ENVMAP(envmap_type)(
			...
			bump_normal,
			view_reflect_dir,
			float4(diffuse_light * Fresnel_Gloss_env, roughness),		// convert specular power to roughness (cheap and bad approximation) EnvBRDFApprox(specular_tint, specular_pbr.roughness, n_dot_v) * 
			1);
	...
	bool cinematic)
{
	...
	if (dot(light_radiance, light_radiance) > 0.0000001f)									// ###ctchou $PERF unproven 'performance' hack
	{
		...
		analytic_diffuse_radiance= light_radiance * dot(fragment_to_light, bump_normal) * diffuse_albedo.rgb;
			...
			//float3 f0= specular_pbr.f0;
			float3 specular_sh = 0.0f;
			analytic_diffuse_radiance *= (1 - fresnel_analytical) * (1 - specular_pbr.metallic);
		#else
			calc_phong_outgoing_light(
				bump_normal,
				view_dir,
